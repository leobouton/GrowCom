...
interface OdooConnectionPanelProps {
  initialConfig: { configured: boolean; odooUrl: string | null; odooLogin: string | null };
  /** Remonte au parent le nouveau statut « connecté » (pour le badge sur le logo). */
  onConfiguredChange: (configured: boolean) => void;
}

export function OdooConnectionPanel({ initialConfig, onConfiguredChange }: OdooConnectionPanelProps) {
  const [configured, setConfigured] = useState(initialConfig.configured);
  const [odooUrl, setOdooUrl]       = useState<string | null>(initialConfig.odooUrl);
  const [odooLogin, setOdooLogin]   = useState<string | null>(initialConfig.odooLogin);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <h2 className="text-base font-semibold text-gray-900 mb-4">Configuration de la connexion Odoo</h2>
